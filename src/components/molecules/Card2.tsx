...
export default function Card1({
  ...
  return (
    <div
      className="card relative cursor-pointer no-select aspect-[16/9] min-w-[300px] "
      ref={cardRef}
    >
      {/* front side ------------------------------------------------------------------ */}
      <div
        className={clsx(
          `front-side bg-gray-100 flex  h-full overflow-hidden   hover:drop-shadow-custom`,
          { "rounded-xl": rounded === "lg" }
        )}
        ref={frontSideRef}
      >
        <div
          className="head relative w-2/6 flex flex-col gap-2 p-4 overflow-hidden"
          style={{
            backgroundColor: `var(--${color1})`,
          }}
        >
          <div className="flex flex-col gap-2 z-10 justify-end h-full">
            <LikeLine stroke={`white`} />

            <div className="flex flex-col flex-1 justify-end ">
              <h3 className="text-white text-lg font-black">{holderName}</h3>
          ...
          <div className="w-[128px] h-[128px] bg-white absolute opacity-25 rounded-[40px] bottom-[-80px] right-[-4px] rotate-[20deg]"></div>
          <div className="w-[180px] h-[180px] bg-white absolute opacity-25 rounded-[40px] bottom-[-100px] right-[-4px] rotate-[30deg]"></div>
        </div>

        <div
          className="w-[5px] h-4/6 opacity-50 rounded-bl-lg"
          style={{
            backgroundColor: `var(--${color1})`,
          }}
        ></div>
        <div className="flex-1 flex flex-col justify-center gap-2 z-10 p-4 ">
          <MyAddress
        ...
        <div className="flex flex-col justify-end">
          <div
            className="w-[5px] h-2/6 opacity-50 rounded-tr-lg"
            style={{
              backgroundColor: `var(--${color1})`,
            }}
          ></div>
      ...
      <div
        className={clsx(
          `back-side absolute top-0 h-full flex items-center justify-center  gap-3   hover:drop-shadow-custom p-8 `,
          { "rounded-xl": rounded === "lg" }
        )}
        style={{
          backgroundColor: `var(--${color1})`,
        ...
        ref={backSideRef}
      >
        <div className="flex p-2 bg-white rounded-lg overflow-hidden h-full flex-col max-w-[88px]">
